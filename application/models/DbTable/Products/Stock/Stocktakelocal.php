...
class Model_DbTable_Products_Stock_Stocktakelocal extends Zend_Db_Table_Abstract {
	
	protected $_name = 'products_v1_stock_take';
...
public function getStocktake( $idTake){
	...
	public function addStocktake( $barcode , $salesYest,$qtySys , $qtyAct , $dateTake , $timeUpload , $location , $staffName){
	...
	public function updateSysStock($qtySys,$date,$location,$bcode){
		
		$whereStr =  "`date_take`= '".$date."' AND `location` LIKE '".$location."' AND `barcode` LIKE '".$bcode."'";
		$data = array('qty_sys' => $qtySys);
